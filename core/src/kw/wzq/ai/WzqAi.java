...
public class WzqAi {
    ...
    public WzqAi() {
        ...
        this.maxX = 15;
        this.maxY = 15;
        this.allFreePoints = createEmptyAll();
        this.myPoints.clear();
    }

    public ArrayList<Point> createEmptyAll(){
        ArrayList<Point> allFree = new ArrayList<>();
        for (int i = 0; i < maxY; i++) {
            for (int i1 = 0; i1 < maxX; i1++) {
                allFree.add(new Point(i,i1));
            }
        }
        return allFree;
    }
    ...
    private Point doAnalysis(List<Point> mySelfPoints, List<Point> oppontPoints) {
        ...
        }else if (oppontPoints.size() == 1) {
            return getFirstPoint(oppontPoints);
        }
        initRange(mySelfPoints, oppontPoints);
        initAnalysisResults();
        Point doFirstAnalysis = doFirstAnalysis(mySelfPoints, oppontPoints);
    ...
    private Point getFirstPoint(List<Point> list) {
        ...
        if (point.getX() == 0
                || point.getY() == 0
                || ((point.getX() == this.maxX && point.getY() == this.maxY))) {
            return new Point(this.maxX / 2, this.maxY / 2);
        }
    ...
    protected class SencondAnalysisResult implements Comparable<SencondAnalysisResult> {
        ...
        public int compareTo(SencondAnalysisResult sencondAnalysisResult) {
            return compareTowResult(this, sencondAnalysisResult);
        }
    }
    ...
    public static void main(String[] args) {
        WzqAi wzqAi = new WzqAi();
        ArrayList<Point> list1 = new ArrayList<>();
        ArrayList<Point> list2 = new ArrayList<>();
        Scanner scanner = new Scanner(System.in);
        while (true) {
            Point point = wzqAi.doAnalysis(list1, list2);
            System.out.println(point);
            list1.add(point);
            wzqAi.allFreePoints.remove(point);
            int x = scanner.nextInt();
            int y = scanner.nextInt();
            Point point1 = new Point(x, y);
            wzqAi.allFreePoints.remove(point1);
