...
public class GameLogicBoard {
    ...
    public GameLogicBoard() {
        //棋盘大小
        gridSize = ConstanNum.GRID_NUMBER;
        zobirst = new Zobirst();
    ...
    //重新初始化
    public void init() {
        gridBoard = new int[gridSize][gridSize];
        comScore = new int[gridSize][gridSize];
        humScore = new int[gridSize][gridSize];
    ...
    public void remove(int x, int y) {
        int role = gridBoard[x][y];
        zobirst.go(x, y, role);
        gridBoard[x][y] = ConstanNum.EMPTY;
        updateScore(x, y);
    ...
    private ArrayList<Point> gen(int role, boolean onlyThrees, boolean starSpread) {
        ...
        for (int i = 0; i < gridBoard.length; i++) {
            for (int j = 0; j < gridBoard.length; j++) {
                if (gridBoard[i][j] == ConstanNum.EMPTY) {
                    if (allSteps.size() < 6) {
    ...
    private int evaluateGame(int role) {
        ...
        for (int i = 0; i < gridBoard.length; i++) {
            for (int j = 0; j < gridBoard[i].length; j++) {
                if (gridBoard[i][j] == ConstanNum.COM) {
                    comMaxScore += fixScore(comScore[i][j]);
                } else if (gridBoard[i][j] == ConstanNum.HUMEN) {
    ...
    private int countToScore(int lineCount, int block, int empty) {
        //没有空位
        if (empty <= 0) {
            return lianxu(lineCount, block);
        } else if (empty == 1 || empty == lineCount - 1) {
            //第一个是空位,如上 分别判断情况
            //成5
            if (lineCount >= 6) {
            ...
            if (block == 0) {
                ...
                switch (lineCount) {
                    case 2:
                        return ConstanNum.TWO / 2;
                    case 3:
                        return ConstanNum.THREE;
                    case 4:
                        return ConstanNum.BLOCKED_FOUR;
                    case 5:
                        return ConstanNum.FOUR;
                }
            }else if (block == 1) {
                ...
                switch (lineCount) {
                    case 2:
                        return ConstanNum.BLOCKED_TWO;
                    case 3:
                        return ConstanNum.BLOCKED_THREE;
                    case 4:
                        return ConstanNum.BLOCKED_FOUR;
                    case 5:
                        return ConstanNum.BLOCKED_FOUR;
        ...
        } else if (empty == 2 || empty == lineCount - 2) {
            //第二个是空位
            //成5
            if (lineCount >= 7) {
                return ConstanNum.FIVE;
        ...
        } else if (empty == 3 || empty == lineCount - 3) {
            //第三个是空位
            if (lineCount >= 8) {
                return ConstanNum.FIVE;
            }
        ...
        } else if (empty == 4 || empty == lineCount - 4) {
            //第四个是空位
            if (lineCount >= 9) {
                return ConstanNum.FIVE;
            }
        ...
        } else if (empty == 5 || empty == lineCount - 5) {
            //第五个是空位
            return ConstanNum.FIVE;
        }
        //无法成5的棋型则返回0
    ...
    private static int lianxu(int lineCount, int block) {
        //成五
        if (lineCount >= 5) {
            return ConstanNum.FIVE;
        }
        ...
        if (block == 0) {
            switch (lineCount) {
                case 1:
                    return ConstanNum.ONE;
                case 2:
                    return ConstanNum.TWO;
                case 3:
                    return ConstanNum.THREE;
                case 4:
                    return ConstanNum.FOUR;
