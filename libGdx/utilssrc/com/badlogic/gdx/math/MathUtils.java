...
public final class MathUtils {
	...
	public static boolean between(int start, int end, int target) {
		if (start<target){
			if (end>=target){
				return true;
			}
	...
	static public float length(float x1,float y1,float x2,float y2){
		float x = x2-x1;
		float y = y2-y1;
	...
	static public float returnValue(float min,float max,float num){
//        if (angle > 173){
//            angle = 173;
//        }
//        if(angle < 7){
...
//        }
	...
	public static int randomRangle(int size){
		return (int)(random() *2);
	}
