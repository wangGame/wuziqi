		...
		for (int level = 0; level < numberOfMipmapLevels; level++) {
			...
			for (int face = 0; face < numberOfFaces; face++) {
				...
				if (textureDimensions == 1) {
					// if (compressed)
					// Gdx.gl.glCompressedTexImage1D(target + face, level1, glInternalFormat, pixelWidth, 0, faceLodSize,
					// data);
					// else
					// Gdx.gl.glTexImage1D(target + face, level1, glInternalFormat, pixelWidth, 0, glFormat, glType, data);
					...
					// if (compressed)
					// Gdx.gl.glCompressedTexImage3D(target + face, level1, glInternalFormat, pixelWidth, pixelHeight, pixelDepth, 0,
					// faceLodSize, data);
					// else
					// Gdx.gl.glTexImage3D(target + face, level1, glInternalFormat, pixelWidth, pixelHeight, pixelDepth, 0, glFormat,
					// glType, data);
				}
			}
		}
